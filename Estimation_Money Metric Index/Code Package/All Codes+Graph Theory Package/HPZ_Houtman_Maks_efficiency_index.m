function [HM,exact] = HPZ_Houtman_Maks_efficiency_index (Choices)
...
% First we go with the exhastive search.
...
for j=(trials-1):-1:number_trials
    ...
    for k=1:rows
      ...
      % third row in of Choices matrix). 
      ...
      % the bundle j (for a Threshold small enough). See detailed explanation
      % in HPZ_Subject_Consistency

      new_DRP = ceil((new_REF+Threshold)/(max(max(abs(new_REF+Threshold)))+1));
      ...
      new_SDRP = ceil((new_REF-Threshold)/(max(max(abs(new_REF-Threshold)))+1));
...
HM_Vector = HoutmanMaksCFGK_2G(input);

%the HM index is calculated as the number of removed relatively to the number of 
% observations.

HM = HM_Vector(1,1)/trials;
