function Index = HPZ_Afriat_efficiency_index (expenditure)
...
for i=1:num_of_iterations
    ...
    % K*(max(max(abs(REF+Threshold)))+1)= REF+Threshold.
    % So, DRP(i,j)=1 iff 0<K(i,j)<=1.

    % Note that:
    % a. max(max(abs(REF+Threshold))) is a positive number, 
    %    because   REF(i,i)=0 and  Threshold>0. Therefore, 
    %    max(max(abs(REF+Threshold)))+1 >1.  
    % b. For every i,j:
    %    max(max(abs(REF+Threshold)))+1 >   (REF+Threshold)(i,j) 
    %    conclusion:(REF+Threshold)(i,j)>0 iff 0<K(i,j)<1.  
