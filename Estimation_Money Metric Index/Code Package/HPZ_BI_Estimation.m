function [param_1,param_2,final_output] = HPZ_BI_Estimation (data,obs_num,treatment,function_flag,beta_flag,zeros_flag,pref_class)
...
%options = optimoptions(@patternsearch,'UseCompleteSearch',true,'Display','off');
%options = psoptimset('CompleteSearch','on','Display','off');
...
optimal_parameter_matrix = zeros(BI_min_counter, 3);

if pref_class == 1
    
    [criterion_EU] = HPZ_BI_Criterion([0 0],endowment,subject_data,treatment,function_flag,pref_class);
...
elseif pref_class == 2
    ...
    [criterion_FCB] = HPZ_BI_Criterion([0.5 0],endowment,subject_data,treatment,function_flag,pref_class);
...
time_optim_conv_pnt = zeros(1, BI_min_counter);
...
for j=1:max_starting_points
    ...
    if BI_time == Inf
        ...
        if (equal_fval_counter < BI_min_counter)
            

            %% Updating the waitbar
            if equal_fval_counter > 0 
                % update the value of the progress bar
                new_bar_val = equal_fval_counter / BI_min_counter;
            end
            ...
            if pref_class == 2
                ...
                [results(j,:),criterion(j)]=patternsearch(@(param) HPZ_BI_Criterion(param,endowment,subject_data,treatment,function_flag,pref_class),initial_points(j,:),[],[],[],[],[0 -inf], [1 1],[],options);
            ...
            elseif beta_zero_flag == true && restricted_rho == false
                ...
                [results(j,2),criterion(j)]=patternsearch(@(param) HPZ_BI_Extreme_Param(param,endowment,subject_data,treatment,function_flag,pref_class),initial_points(j,2),[],[],[],[], 0, [],[],options);
            ...
            elseif beta_zero_flag == true && restricted_rho == true
                ...
                [results(j,2),criterion(j)]=patternsearch(@(param) HPZ_BI_Extreme_Param(param,endowment,subject_data,treatment,function_flag,pref_class),initial_points(j,2),[],[],[],[], 0, 1,[],options);

            elseif beta_flag==2 && restricted_rho == false
                [results(j,:),criterion(j)]=patternsearch(@(param) HPZ_BI_Criterion(param,endowment,subject_data,treatment,function_flag,pref_class),initial_points(j,:),[],[],[],[],[0 0], [],[],options);
            ...
            elseif beta_flag==2 && restricted_rho == true
                ...
                [results(j,:),criterion(j)]=patternsearch(@(param) HPZ_BI_Criterion(param,endowment,subject_data,treatment,function_flag,pref_class),initial_points(j,:),[],[],[],[],[0 0], [inf 1],[],options);

            elseif beta_flag==1 && restricted_rho == false
                % [results(j,:),criterion(j)]=fminsearchbnd(@(param) HPZ_BI_Criterion(param,endowment,subject_data,treatment,function_flag,pref_class),initial_points(j,:),[-1 0], [], options);
                [results(j,:),criterion(j)]=patternsearch(@(param) HPZ_BI_Criterion(param,endowment,subject_data,treatment,function_flag,pref_class),initial_points(j,:),[],[],[],[],[-1 0], [],[],options);
                
            elseif beta_flag==1 && restricted_rho == true
                % if the subject has a corner solution(s) where CRRA is chosen 
                % as the utility function, then restrict rho<1 and beta can be
                % a negative number [-1, inf]
                % [results(j,:),criterion(j)]=fminsearchbnd(@(param) HPZ_BI_Criterion(param,endowment,subject_data,treatment,function_flag,pref_class),initial_points(j,:),[-1 0], [inf 1], options);
                [results(j,:),criterion(j)]=patternsearch(@(param) HPZ_BI_Criterion(param,endowment,subject_data,treatment,function_flag,pref_class),initial_points(j,:),[],[],[],[],[-1 0], [inf 1],[],options);
...
%             end
            ...
            if equal_fval_counter == BI_min_counter || j == max_starting_points
        ...
        if BI_time > (time_init_accum + time_init_pt)
...
if BI_time == Inf
	...
	for i = 1 : max_index
        ...
        %action_flag = 3; %% MME
        %[fid_BI_final_report] = HPZ_Write_Result_File_Finalizer(action_flag, i , fid_BI_final_report, final_output, num2str(data(1,1)), output_flag_vec);
        
        % update the waitbar
        waitbar(new_bar_val + ((i/BI_min_counter)*remaindar_multplier));
	end
    
else
    %% time limited estimation
    
    num_of_points = min(total_init_points,BI_min_counter);
