...
%% Setup
data = readtable('data_PE_numerical.csv');
...
for i=1:I
    ...
    u_xc=-alpha*exp(-rho*min(xo(i,:)))-(1-alpha)*exp(-rho*max(xo(i,:))); % utility at the observed choice
...
ite(i)=j;
u(i,:)=[u_xc,-alpha*exp(-rho*min(xl(i,:)))-(1-alpha)*exp(-rho*max(xl(i,:))),...
              -alpha*exp(-rho*min(xu(i,:)))-(1-alpha)*exp(-rho*max(xu(i,:)))];
