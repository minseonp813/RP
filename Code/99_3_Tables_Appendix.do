...
foreach measure in hm maxmpi {
    if "`measure'" == "hm" {
        ...
        local gap "hm_gap_ij"
        label var HighHM_both_high "\$Higher (1-HM)_i\$"
        label var HighHM_both_low "\$Higher (1-HM)_i\$"
        label var hm_gap_ij "\$(1-HM)_i-(1-HM)_j\$"
    }
    else {
        local title "MaxMPI"
        ...
        label var HighMaxMPI_both_high "\$Higher (1-MaxMPI)_i\$"
        label var HighMaxMPI_both_low "\$Higher (1-MaxMPI)_i\$"
        label var maxmpi_gap_ij "\$(1-MaxMPI)_i-(1-MaxMPI)_j\$"
    }
    ...
    eststo clear
    eststo ah1: reghdfe `outcome' `high' if balanced_alt, absorb(class) vce(cluster class)
    * eststo ah2: reghdfe `outcome' `high' $ai_group $ai_friend $ai_missing if balanced_alt, absorb(class) vce(cluster class)
    eststo ah3: reghdfe `outcome' `high' $ai_group $ai_friend $ai_missing $ai_ra $ai_share if balanced_alt, absorb(class) vce(cluster class)
    eststo ah4: reghdfe `outcome' `high' $ai_group_nogender $ai_friend $ai_missing $ai_ra $ai_share if balanced_alt, absorb(id_fe) vce(cluster class)
    eststo ad1: reghdfe `outcome' `gap' if balanced_alt, absorb(class) vce(cluster class)
    * eststo ad2: reghdfe `outcome' `gap' $ai_group $ai_friend $ai_missing if balanced_alt, absorb(class) vce(cluster class)
    eststo ad3: reghdfe `outcome' `gap' $ai_group $ai_friend $ai_missing $ai_ra $ai_share if balanced_alt, absorb(class) vce(cluster class)
    eststo ad4: reghdfe `outcome' `gap' $ai_group_nogender $ai_friend $ai_missing $ai_ra $ai_share if balanced_alt, absorb(id_fe) vce(cluster class)
    foreach m in ah1 ah3 ad1 ad3 {
        estadd local fixed_effects "Class" : `m'
    }
    foreach m in ah4 ad4 {
        estadd local fixed_effects "Individual" : `m'
    }
    foreach m in ah3 ah4 ad3 ad4 {
        estadd local individual_controls "\checkmark" : `m'
        estadd local friendship_controls "\checkmark" : `m'
        estadd local ra_controls "\checkmark" : `m'
        estadd local share_controls "\checkmark" : `m'
    }
    esttab ah1 ah3 ah4 ad1 ad3 ad4 using `"`tex_dir'/table_bargaining`title'.tex"', replace ///
        b(3) se(3) stats(N r2 fixed_effects individual_controls friendship_controls ra_controls share_controls, ///
        ...
        nogap compress star(+ 0.1 * 0.05 ** 0.01) label substitute(\_ _) ///
        keep(`high' `gap' mathscore_i mathscore_diff inclass_popularity_i inclass_pop_diff female_i_male_j male_i_female_j) ///
