********************************************************************************
* 99_Tables_Appendix.do
* Appendix tables and the Stata-side inputs for appendix Shapley figures.
* Existing 99_* files are intentionally left unchanged.
*
...
label var I_ig "Revealed Preference Distance Index"
...
local col_vars "ccei_i RA_i I_ig inclass_n_friends_i inclass_popularity_i"
local group1_vars "ccei_i RA_i I_ig"
...
file open corrfile using `"`tex_dir'/table_correlation.tex"', write replace
...
bysort id: egen n_main_I_ig = total(!missing(I_ig))
gen byte balanced_main = (n_main_I_ig == 2)

reghdfe I_ig HighCCEI $ai_group_nogender $ai_friend $ai_missing $ai_ra $ai_share ///
...
reg I_ig `G_CCEI_T3' `G_CHAR_T3' `G_RA_T3' `G_SHARE_T3' `G_INDIVIDUAL_FE' ///
...
egen total_r2 = total(shapley_value)
export excel block shapley_value shapley_percent total_r2 ///
    using `"`result_dir'/shapley_bargaining_index.xlsx"', firstrow(variables) replace
export delimited block shapley_value shapley_percent total_r2 ///
    using `"`result_dir'/shapley_bargaining_index.csv"', replace
restore

di as result "99_3_Tables_Appendix.do completed."
