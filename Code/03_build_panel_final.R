...
baseline_only_base_cols <- paste0(baseline_only_noncog_stems, "_base")
if (!all(baseline_only_base_cols %in% names(noncog_pre_clean))) {
  stop("Missing baseline-only noncognitive variables in noncog_pre_clean.")
}

noncog_baseline_for_end <- noncog_pre_clean %>%
  select(id, all_of(baseline_only_base_cols))
names(noncog_baseline_for_end) <- str_replace(
  names(noncog_baseline_for_end),
  "_base$",
  "_end"
)
...
module_specs <- list(
  ...
  list(df = noncog_baseline_for_end, id_col = "id_mover_end", person = 1),
  list(df = noncog_baseline_for_end, id_col = "id_nonmover_end", person = 2),
...
stopifnot(
  all(panel_final$cei_g_base > 0 & panel_final$cei_g_base <= 1),
  all(panel_final$cei_g_end > 0 & panel_final$cei_g_end <= 1),
  all(panel_final$cei_g_untempered_base > 0 & panel_final$cei_g_untempered_base <= 1),
  all(panel_final$cei_g_untempered_end > 0 & panel_final$cei_g_untempered_end <= 1)
)

for (suffix in c("base", "end")) {
  ccei_1 <- panel_final[[paste0("ccei_1_", suffix)]]
  ccei_2 <- panel_final[[paste0("ccei_2_", suffix)]]
  defined <- !is.na(ccei_1) & !is.na(ccei_2)

  panel_final[[paste0("HighCCEI_1_both_high_", suffix)]] <-
    ifelse(defined, as.numeric(ccei_1 >= ccei_2), NA_real_)
  panel_final[[paste0("HighCCEI_2_both_high_", suffix)]] <-
    ifelse(defined, as.numeric(ccei_2 >= ccei_1), NA_real_)
  panel_final[[paste0("HighCCEI_1_both_low_", suffix)]] <-
    ifelse(defined, as.numeric(ccei_1 > ccei_2), NA_real_)
  panel_final[[paste0("HighCCEI_2_both_low_", suffix)]] <-
    ifelse(defined, as.numeric(ccei_2 > ccei_1), NA_real_)
  panel_final[[paste0("ccei_gap_12_", suffix)]] <- ccei_1 - ccei_2
  panel_final[[paste0("ccei_gap_21_", suffix)]] <- ccei_2 - ccei_1

  for (measure in c("maxmpi", "hm")) {
    score_1 <- panel_final[[paste0(measure, "_1_", suffix)]]
    score_2 <- panel_final[[paste0(measure, "_2_", suffix)]]
    score_defined <- !is.na(score_1) & !is.na(score_2)

    label <- if (measure == "maxmpi") "MaxMPI" else "HM"
    panel_final[[paste0("High", label, "_1_both_high_", suffix)]] <-
      ifelse(score_defined, as.numeric(score_1 <= score_2), NA_real_)
    panel_final[[paste0("High", label, "_2_both_high_", suffix)]] <-
      ifelse(score_defined, as.numeric(score_2 <= score_1), NA_real_)
    panel_final[[paste0("High", label, "_1_both_low_", suffix)]] <-
      ifelse(score_defined, as.numeric(score_1 < score_2), NA_real_)
    panel_final[[paste0("High", label, "_2_both_low_", suffix)]] <-
      ifelse(score_defined, as.numeric(score_2 < score_1), NA_real_)
    panel_final[[paste0(measure, "_gap_12_", suffix)]] <- score_2 - score_1
    panel_final[[paste0(measure, "_gap_21_", suffix)]] <- score_1 - score_2
  }
}
...
for (suffix in c("base", "end")) {
  ccei_1 <- panel_final[[paste0("ccei_1_", suffix)]]
  ccei_2 <- panel_final[[paste0("ccei_2_", suffix)]]
  equal <- !is.na(ccei_1) & !is.na(ccei_2) & ccei_1 == ccei_2
  stopifnot(
    all(panel_final[[paste0("HighCCEI_1_both_high_", suffix)]][equal] == 1),
    all(panel_final[[paste0("HighCCEI_2_both_high_", suffix)]][equal] == 1),
    all(panel_final[[paste0("HighCCEI_1_both_low_", suffix)]][equal] == 0),
    all(panel_final[[paste0("HighCCEI_2_both_low_", suffix)]][equal] == 0),
    all(
      panel_final[[paste0("ccei_gap_12_", suffix)]] ==
        -panel_final[[paste0("ccei_gap_21_", suffix)]],
      na.rm = TRUE
    )
  )
  ...
  for (measure in c("maxmpi", "hm")) {
    ...
    stopifnot(
      all(panel_final[[paste0("High", label, "_1_both_high_", suffix)]][equal] == 1),
      all(panel_final[[paste0("High", label, "_2_both_high_", suffix)]][equal] == 1),
      all(panel_final[[paste0("High", label, "_1_both_low_", suffix)]][equal] == 0),
      all(panel_final[[paste0("High", label, "_2_both_low_", suffix)]][equal] == 0),
      all(
        panel_final[[paste0(measure, "_gap_12_", suffix)]] ==
          -panel_final[[paste0(measure, "_gap_21_", suffix)]],
        na.rm = TRUE
      )
    )
  }
