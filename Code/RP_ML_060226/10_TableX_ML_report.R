...
#
# Intentional differences from the replication code:
#   1) for pair-level controls, we use *_i and *_diff = *_i - *_j
#   2) create missing dummies for numeric pair-level controls, then zero-variance/perfect-collinearity vars are removed
#   3) Table5 prints the full CLAN table instead of splitting main/appendix parts
#--------------------------------------------------------------------------------------------------------------------
...
cl   <- makeCluster(8, outfile="")   # 8 workers: caps concurrent (X)^2 matrices to fit 32GB RAM
registerDoParallel(cl)

####################################### Output Paths  #######################################

# Save outputs inside the current working directory.
table_dir  <- "Tables"
figure_dir <- "Figures"
...
print(normalizePath(table_dir, mustWork = FALSE))

cat("\nFigures will be saved to:\n")
print(normalizePath(figure_dir, mustWork = FALSE))
...
# Keep row id and original dataset for merging back heterogeneity scores later.
...
cluster_var      <- "class_i"
fixed_effect_var <- "class_i"

# Make treatment numeric 0/1
D_raw <- data[[d_var]]

if(inherits(D_raw, "labelled")){
  D_raw <- haven::as_factor(D_raw)
}
if(is.factor(D_raw)){
  D_raw <- as.character(D_raw)
}

D_num <- suppressWarnings(as.numeric(D_raw))

if(all(is.na(D_num))){
  D_num <- ifelse(
    is.na(D_raw),
    NA_real_,
    ifelse(D_raw %in% c("0", "FALSE", "False", "false"), 0, 1)
  )
}

data[[d_var]] <- ifelse(is.na(D_num), NA_real_, ifelse(D_num != 0, 1, 0))

# Make outcome numeric
data[[y_var]] <- as.numeric(data[[y_var]])

# No missing dummies are created for malepair, friendship, post, or class_i.
if("malepair" %in% colnames(data)){
  data$malepair <- suppressWarnings(as.numeric(data$malepair))
}

if("friendship" %in% colnames(data)){
  data$friendship <- suppressWarnings(as.numeric(data$friendship))
}

if("post" %in% colnames(data)){
  data$post <- suppressWarnings(as.numeric(data$post))
}

if("class_i" %in% colnames(data)){
  data$class_i <- suppressWarnings(as.numeric(data$class_i))
}

####################################### Inputs  #######################################

sim     <- 100     # reporting run: median over 100 sample splits
K       <- 2       # number of folds
...
# The rule is:
# For every numeric variable x, we use:
#   1) own value: x_i
#   2) pair difference: x_diff = x_i - x_j
#
# No missing dummies are created for malepair, friendship, post, or class_i.
...
pair_var_stems <- c(
  ...
  "pblclass_horizontal",
  "pblclass_horizontal_q1",
  
  "pbleffect_sharidea",
  "pbleffect_helpstudy",
  "pbleffect_helpnonstudy",
  "pbleffect_oldfriend",
  "pbleffect_newfriend",
  "pbleffect_inclass",
  "pbleffect_outclass",
  "pbleffect_friend",
  "pbleffect_confidence",
  "pbleffect_school",
...
for(v in pair_var_stems){
  
  vi    <- paste0(v, "_i")
  vj    <- paste0(v, "_j")
  vdiff <- paste0(v, "_diff")
  
  if(vi %in% colnames(data)){
    data[[vi]] <- suppressWarnings(as.numeric(data[[vi]]))
    pair_controls <- c(pair_controls, vi)
  }
  
  if(all(c(vi, vj) %in% colnames(data))){
    data[[vj]] <- suppressWarnings(as.numeric(data[[vj]]))
    data[[vdiff]] <- data[[vi]] - data[[vj]]
    pair_controls <- c(pair_controls, vdiff)
  }
}

# These are not included in the missing-dummy/imputation procedure.
as_is_controls <- c("malepair", "friendship", "post")
as_is_controls <- as_is_controls[as_is_controls %in% colnames(data)]

for(v in as_is_controls){
  data[[v]] <- suppressWarnings(as.numeric(data[[v]]))
}

controls_numeric_raw <- unique(pair_controls)

controls_as_is_raw <- unique(as_is_controls)

####################################### Missing Dummies + Imputation  #######################################

# For every numeric pair-level control:
#   1) create missing dummy
#   2) impute missing values to 0
#
# No missing dummies are created for malepair, friendship, post, or class_i.
...
for(v in controls_numeric_raw){
  ...
  miss_v <- paste0(v, "_missing")
  data[[miss_v]] <- as.numeric(is.na(data[[v]]))
  data[[v]][is.na(data[[v]])] <- 0
  
  missing_vars <- c(missing_vars, miss_v)
}
...
controls <- controls[controls %in% colnames(data)]


####################################### Variables for CLAN  #######################################

# class_i is not included in CLAN affected variables because it is used only as fixed effect and clustering.
...
affected <- affected[affected %in% colnames(data)]

names_affected <- affected


####################################### Generate Formula X and XL  #######################################
...
######################################################################################################
# Keep only variables needed for the ML / GATES / BLP / CLAN procedure
######################################################################################################
...
####################################### Collinearity Diagnostics and Dropping  #######################################

# This block reports zero-variance variables, perfectly correlated pairs,
# and exact linear combinations in the model matrix.
# Zero-variance variables and variables suggested by caret::findLinearCombos()
# are automatically removed before ML estimation.
...
####################################### Collinearity Dropping  #######################################

# This block applies the removal rule described above:
# 1) remove zero-variance numeric controls
# 2) remove variables suggested by caret::findLinearCombos()
...
affected <- setdiff(affected, remove_controls)
names_affected <- affected
...
# Build the squared-interaction design ONCE, then drop zero-variance and
# duplicate interaction columns before glmnet. This is result-neutral
# (constant / duplicate columns carry no information and caret+glmnet would
# discard them anyway) but eliminates the thousands of preProcess warnings
# and shrinks the design matrix -- the real glmnet bottleneck.
XL_full_form <- as.formula(paste("~ (", X, ")^2", sep=""))
XL_mm        <- model.matrix(XL_full_form, data = data)
XL_mm        <- XL_mm[, colnames(XL_mm) != "(Intercept)", drop = FALSE]

XL_keep      <- apply(XL_mm, 2, function(z) length(unique(z)) > 1)   # drop zero-variance
XL_mm        <- XL_mm[, XL_keep, drop = FALSE]
XL_dup       <- duplicated(t(XL_mm))                                 # drop duplicate columns
XL_terms     <- colnames(XL_mm)[!XL_dup]

XL <- paste(XL_terms, collapse = "+")

cat("\nXL interaction terms kept after pruning (was ", ncol(XL_mm) + sum(XL_dup) + sum(!XL_keep), "):\n", sep="")
print(length(XL_terms))

rm(XL_mm); gc()

# Rebuild key_variables
key_variables <- c(Y, D, controls)
...
# Original empirical four-method set.
...
args         <- list(svmLinear2=list(type='eps-regression'),
                     ...
                     pcaNNet=list(linout = TRUE, trace = FALSE, MaxNWts=100000, maxit=1000),
...
r <- foreach(t = 1:sim, .combine='cbind', .inorder=FALSE, .packages=vec.pac) %dopar% {
  ...
  for(i in 1:length(Y)){
    ...
    cc_vars <- c(controls, y, d, affected)
    ...
    for(l in 1:length(methods)){
      ...
      for(m in 1:length(affected)){
        a  <- tryCatch({
          form  <- as.formula(paste(affected[m],"~h+l-1", sep=""))
          ...
          cat("ERROR3 :",methods[l], t, i, affected[m], "\n")
          
          res1  <- c(mean(dataout[(dataout$h==1),affected[m]], na.rm=TRUE), mean(dataout[(dataout$h==1),affected[m]], na.rm=TRUE), mean(dataout[(dataout$h==1),affected[m]], na.rm=TRUE), 0.5, 0.5)
          res2  <- c(mean(dataout[(dataout$l==1),affected[m]], na.rm=TRUE), mean(dataout[(dataout$l==1),affected[m]], na.rm=TRUE), mean(dataout[(dataout$l==1),affected[m]], na.rm=TRUE), 0.5, 0.5)
...
bestML_all_inter  <- t(sapply(seq(1:nrow(bestML[,,1])), function(x) apply(t(bestML[x,,]),2, quantile, probs = c(0.25,0.75))))
...
# After selecting the best ML method, compute one observation-level heterogeneity score.
# It is then merged back to the original dataset by row_id and saved separately.
...
score_vars <- unique(c("row_id", y_score, d_score, controls, affected))
...
save(data_scored, file=file.path(table_dir, "data_with_het_score.RData"))
...
print(xtable(cbind(rownames(best_ML),best_ML)), include.rownames=FALSE,file=file.path(table_dir, "Table2.txt"), digits=3)
print(xtable(cbind(rownames(BLP_final),BLP_final)), include.rownames=FALSE,file=file.path(table_dir, "Table3.txt"), digits=3)
print(xtable(cbind(rownames(GATES_final),GATES_final)), include.rownames=FALSE,file=file.path(table_dir, "Table4-1.txt"), digits=3)
print(xtable(cbind(rownames(CLAN_final),CLAN_final)), include.rownames=FALSE,file=file.path(table_dir, "Table5.txt"), digits=3)
...
for(i in 1:length(Y)){
  ...
  ggsave(file.path(figure_dir, "Figure4.pdf"), p_best, height=3.5, width=7)
}

save.image(file='datas.RData')
stopCluster(cl)
