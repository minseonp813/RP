...
results=zeros(n,10); 
    ...
    aa = [0.05 3]; 
    nar = size(b',1)* size(d',1) * size(aa',1) ;
    vpm = zeros(nar,3); vssr = zeros(nar,1); 
    tvar={b,d,aa};
    [bb, dd, aaa]=ndgrid(tvar{:});
    var=[bb(:), dd(:), aaa(:)];
    clear bb dd aaa tvar;
...
%    sst = sum((y-x).^2); r2 = 1-ssr/sst;
    ...
    for j = 1:M
        ...
        pd = opdem(beta,delta,A,mE,mL,mT);
        
        Pred = [Pred; id, j, mT, pd(1),pd(2), mE, mL]; 
    end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    

    results(i,:) = [id,beta,se_beta,delta,se_delta,cov_bd,A, se_A, ssr, M];
