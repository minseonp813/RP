...
for i =  1:1.5*N
    ...
    % bt_results = [bt_results; id,zeros(1,2)];  
     
    %elseif sum((0.45<ry).*(ry<0.55)) >= 18
    %alpha=0.5; se1=0; rho=10; se2=0;
    %ssr=sum(abs(ry-0.5));
    %results(i,:) = [VarName1(m1),VarName7(m1),alpha,se1,rho,se2,ssr];
    %bt_results = [bt_results; id,zeros(1,2)];  
...
m1=m1+M ;     
     ...
     save bt_results_Pre_20170605 bt_results
     save results_Pre_20170605 results
