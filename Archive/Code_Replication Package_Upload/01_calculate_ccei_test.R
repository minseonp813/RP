...
write_dta(
  base_raw,
  "data/base_raw_test.dta"
)
write_dta(
  end_raw,
  "data/end_raw_test.dta"
)
############################################################
...
source("programs/ex_cross_fgarp.R")
source("programs/ex_cross_rev_max_mpi.R")

base_raw <- read_dta("data/base_raw_test.dta")
end_raw <- read_dta("data/end_raw_test.dta")
panel_final <- read_dta("data/panel_final_test.dta")
...
old_generated_vars <- grep(
  "^(ccei_|I_hg_|I_lg_|Ihat_|RA_|high_|f_ccei_|f_I_|f_high_|max_mpi_|rev_max_mpi_|I_rev_max_mpi_|I_max_mpi_)",
  names(panel_final),
  value = TRUE
)
...
compute_fgarp_wave_measures <- function(raw, panel, suffix, id_mover_col, id_nonmover_col) {
  ...
  fIhat_hg_col <- paste0("f_Ihat_hg_", suffix)
  fIhat_lg_col <- paste0("f_Ihat_lg_", suffix)
  ...
  ## ---- BEGIN cross-partition FGARP index ----
  panel[[fIhat_hg_col]] <- NA_real_
  panel[[fIhat_lg_col]] <- NA_real_
  ...
  for (i in seq_len(nrow(panel))) {
    ...
    f_ex_h <- compute_f_ex_from_subsets(
      high_indiv,
      group_side,
      fgarp_pi
    )
    f_ex_l <- compute_f_ex_from_subsets(
      low_indiv,
      group_side,
      fgarp_pi
    )
    f_ex_hl <- compute_f_ex_from_subsets(
      both_indiv,
      group_side,
      fgarp_pi
    )

    panel[[fIhat_hg_col]][i] <- f_ihat_from_ex(
      f_ex_h,
      f_ex_l,
      f_ex_hl
    )
    panel[[fIhat_lg_col]][i] <- f_ihat_from_ex(
      f_ex_l,
      f_ex_h,
      f_ex_hl
    )
  ...
  f_ihat_defined <- !is.na(panel[[fIhat_hg_col]]) &
    !is.na(panel[[fIhat_lg_col]])

  if (any(f_ihat_defined)) {
    f_ihat_max_sum_error <- max(abs(
      panel[[fIhat_hg_col]][f_ihat_defined] +
        panel[[fIhat_lg_col]][f_ihat_defined] - 1
    ))

    f_ihat_in_range <- all(
      panel[[fIhat_hg_col]][f_ihat_defined] >= -1e-8 &
        panel[[fIhat_hg_col]][f_ihat_defined] <= 1 + 1e-8 &
        panel[[fIhat_lg_col]][f_ihat_defined] >= -1e-8 &
        panel[[fIhat_lg_col]][f_ihat_defined] <= 1 + 1e-8
    )

    if (f_ihat_max_sum_error > 1e-8 || !f_ihat_in_range) {
      stop(suffix, ": cross-partition FGARP index validation failed.")
    }
  }
  ...
  cat(
    "\n",
    suffix,
    ": cross-partition FGARP index defined for",
    sum(f_ihat_defined),
    "of",
    nrow(panel),
    "groups.\n"
  )
  ## ---- END cross-partition FGARP index ----
...
compute_rev_max_mpi_wave_measures <- function(raw, panel, suffix, id_mover_col, id_nonmover_col) {
  ...
  Ihat_rev_hg_col <- paste0("Ihat_rev_max_mpi_hg_", suffix)
  Ihat_rev_lg_col <- paste0("Ihat_rev_max_mpi_lg_", suffix)
  ...
  ## ---- BEGIN cross-partition RevMaxMPI index ----
  ## High/low membership follows rev_max_mpi_high_<suffix>, defined above
  ## from individual RevMaxMPI (with the existing combined-data tie-breaker).
  ## Every pair in panel is processed; no sampling is performed.
  panel[[Ihat_rev_hg_col]] <- NA_real_
  panel[[Ihat_rev_lg_col]] <- NA_real_
  ...
  for (i in seq_len(nrow(panel))) {
    ...
    if (is.na(panel[[rev_high_col]][i])) next

    if (panel[[rev_high_col]][i] == 1) {
      high_id <- id1
      low_id <- id2
    } else {
      high_id <- id2
    ...
    high_indiv <- data_indiv[
      data_indiv$group_id == g & data_indiv$id == high_id,
    ]

    low_indiv <- data_indiv[
      data_indiv$group_id == g & data_indiv$id == low_id,
    ]
    ...
    cross_max_h <- compute_cross_max_mpi_from_subsets(
      high_indiv,
      group_side
    )
    cross_max_l <- compute_cross_max_mpi_from_subsets(
      low_indiv,
      group_side
    )
    cross_max_hl <- compute_cross_max_mpi_from_subsets(
      both_indiv,
      group_side
    )

    panel[[Ihat_rev_hg_col]][i] <- ihat_from_cross_max_mpi(
      cross_max_h,
      cross_max_l,
      cross_max_hl
    )
    panel[[Ihat_rev_lg_col]][i] <- ihat_from_cross_max_mpi(
      cross_max_l,
      cross_max_h,
      cross_max_hl
    )
  ...
  rev_ihat_defined <- !is.na(panel[[Ihat_rev_hg_col]]) &
    !is.na(panel[[Ihat_rev_lg_col]])

  if (any(rev_ihat_defined)) {
    rev_ihat_max_sum_error <- max(abs(
      panel[[Ihat_rev_hg_col]][rev_ihat_defined] +
        panel[[Ihat_rev_lg_col]][rev_ihat_defined] - 1
    ))

    rev_ihat_in_range <- all(
      panel[[Ihat_rev_hg_col]][rev_ihat_defined] >= -1e-8 &
        panel[[Ihat_rev_hg_col]][rev_ihat_defined] <= 1 + 1e-8 &
        panel[[Ihat_rev_lg_col]][rev_ihat_defined] >= -1e-8 &
        panel[[Ihat_rev_lg_col]][rev_ihat_defined] <= 1 + 1e-8
    )

    if (rev_ihat_max_sum_error > 1e-8 || !rev_ihat_in_range) {
      stop(suffix, ": cross-partition RevMaxMPI index validation failed.")
    }
  }
  ...
  cat(
    "\n",
    suffix,
    ": cross-partition RevMaxMPI index defined for",
    sum(rev_ihat_defined),
    "of",
    nrow(panel),
    "groups.\n"
  )
  ## ---- END cross-partition RevMaxMPI index ----
...
panel_final %>%
  select(
    ...
    Ihat_rev_max_mpi_hg_base,
    Ihat_rev_max_mpi_lg_base,
    ccei_1_end,
    ccei_2_end,
    ccei_g_end,
    ccei_1g_end,
    ...
    Ihat_rev_max_mpi_hg_end,
    Ihat_rev_max_mpi_lg_end
  ) %>%
  head() %>%
  print()
write_dta(
