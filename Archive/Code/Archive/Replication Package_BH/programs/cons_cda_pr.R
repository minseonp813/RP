cons_cda_pr = function (p, x, L1, L2, pi, gamma, e) {
        gamma.tilde = (gamma[1, 1] * (1 - pi[1, 1]) ^ 2) / (gamma[1, 1] - 2 * gamma[1, 1] * pi[1, 1] + pi[1, 1] ^ 2);
        gamma.tilde = matrix (c (1 - gamma.tilde, gamma.tilde), 2, 1);
        n.states = dim (p)[1];
        n.obs = dim (p)[2];
        x.max = max (ceiling (matrix (apply (p * x, 2, sum), n.states, n.obs, byrow = TRUE) / p)) + 1;
        x.unique = sort (unique (c (0, x, x.max, L1, L2)));
        ...
        for (i in 1 : n.obs) {
                ...
                for (j in 1 : n.keep.i) {
                        if (grid.intersect.i[j, 1] >= grid.intersect.i[j, 2]) {
                                ...
                                for (k in 1 : n.states) {
                                        A.1.1.i[j, map.lb.i[j, k]] = A.1.1.i[j, map.lb.i[j, k]] + lambda.i[j, k] * gamma.tilde[k, 1];
                                        A.1.1.i[j, map.ub.i[j, k]] = A.1.1.i[j, map.ub.i[j, k]] + (1 - lambda.i[j, k]) * gamma.tilde[k, 1];
                                }
                        }
                }
                ...
                if (x.i[1] >= x.i[2]) {
                        ...
                        for (j in 1 : n.states) {
                                A.1.2.i[which (x.unique %in% x.i[j])] = A.1.2.i[which (x.unique %in% x.i[j])] + gamma.tilde[j, 1];
                        }
                }
        ...
        for (i in 1 : n.obs) {
                ...
                if (x.i[1] >= x.i[2]) {
                        ...
                        for (j in 1 : n.states) {
                                A.45.i[which (x.unique %in% x.i[j])] = A.45.i[which (x.unique %in% x.i[j])] + gamma.tilde[j, 1];
                        }
                }
                A.45.i = -A.45.i;
        ...
        A.lottery = rep (0, n.x.unique);
        if (L1[1, 1] >= L1[2, 1]) {
                A.lottery[which (x.unique %in% L1[1, 1])] = A.lottery[which (x.unique %in% L1[1, 1])] - gamma[1, 1];
                A.lottery[which (x.unique %in% L1[2, 1])] = A.lottery[which (x.unique %in% L1[2, 1])] - gamma[2, 1];
        } else {
                A.lottery[which (x.unique %in% L1[1, 1])] = A.lottery[which (x.unique %in% L1[1, 1])] - gamma.tilde[1, 1];
                A.lottery[which (x.unique %in% L1[2, 1])] = A.lottery[which (x.unique %in% L1[2, 1])] - gamma.tilde[2, 1];                
        }
        if (L2[1, 1] >= L2[2, 1]) {
                A.lottery[which (x.unique %in% L2[1, 1])] = A.lottery[which (x.unique %in% L2[1, 1])] + gamma[1, 1];
                A.lottery[which (x.unique %in% L2[2, 1])] = A.lottery[which (x.unique %in% L2[2, 1])] + gamma[2, 1];
        } else {
                A.lottery[which (x.unique %in% L2[1, 1])] = A.lottery[which (x.unique %in% L2[1, 1])] + gamma.tilde[1, 1];
                A.lottery[which (x.unique %in% L2[2, 1])] = A.lottery[which (x.unique %in% L2[2, 1])] + gamma.tilde[2, 1];
        }
        b.lottery = -1;
        A = rbind (A, A.lottery);
        b = c (b, b.lottery);
