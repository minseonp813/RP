cons_rdu = function (p, x, gamma, theta, e) {
        ...
        for (i in 1 : dim (p)[2]) {
                if (x[1, i] >= x[2, i]) {
                        ...
                        A3_below[((i - 1) * dim (grid_below)[1] + 1) : (i * dim (grid_below)[1]), 1 : dim (A3_below)[2]] = t (matrix (t (-((x[2, i] == sort (unique (c (0, x)))) * theta[1, 1] + (x[1, i] == sort (unique (c (0, x)))) * theta[2, 1])), dim (A3_below)[2], dim (grid_below)[1]));
                }
        }
        ...
        for (i in 1 : dim (p)[2]) {
                for (j in 1 : dim (grid_above)[1]) {
                        A1_above[(i - 1) * dim (grid_above)[1] + j,] = (sort (unique (c (0, x))) == grid_above[j, 1]) * theta[2, 1];
                }
        }
        A2_above = matrix (0, dim (A_above)[1], dim (A_above)[2]);
        for (i in 1 : dim (p)[2]) {
                for (j in 1 : dim (grid_above)[1]) {
                        A2_above[(i - 1) * dim (grid_above)[1] + j,] = (sort (unique (c (0, x))) == grid_above[j, 2]) * theta[1, 1];
                }
        }
        ...
        for (i in 1 : dim (p)[2]) {
                if (x[1, i] >= x[2, i]) {
                        ...
                        A3_above[((i - 1) * dim (grid_above)[1] + 1) : (i * dim (grid_above)[1]), 1 : dim (A3_above)[2]] = t (matrix (t (-((x[2, i] == sort (unique (c (0, x)))) * theta[1, 1] + (x[1, i] == sort (unique (c (0, x)))) * theta[2, 1])), dim (A3_above)[2], dim (grid_above)[1]));
                }
        }
