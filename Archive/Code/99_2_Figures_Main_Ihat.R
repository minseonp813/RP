################################################################################
# 99_2_Figures_Main_Ihat.R
# Standalone main-paper figure replication using Ihat_ig where applicable.
#
...
make_figure2_pair <- function(outcome_var, output_dir) {
  ...
  ggsave(
    file.path(
      output_dir,
      "ccei_bargaining_whose_suggestion_Ihat.png"
    ),
    whose_plot,
    width = 7,
    height = 5,
  ...
  ggsave(
    file.path(
      output_dir,
      "ccei_bargaining_had_individual_high_Ihat.png"
    ),
    similar_plot,
    width = 7,
    height = 5,
...
make_figure2_pair(distance_var, ihat_result_dir)
...
rp_data <- panel_individual |>
  filter(!is.na(.data[[distance_var]]), !is.na(HighCCEI)) |>
  transmute(
...
distance_symbol <- if (distance_var == "Ihat_ig") {
  quote(hat(I)[ig])
} else {
  quote(I[ig])
}
...
ggsave(
  ihat_output_path("bargaining_index_by_ccei_bar"),
  rp_bar, width = 6, height = 5, dpi = 300
)
ggsave(
  ihat_output_path("bargaining_index_by_ccei_cdf"),
  rp_cdf, width = 6, height = 5, dpi = 300
)

################################################################################
# Overlay histogram of Ihat_ig by members' CCEI
################################################################################

hist_overlay_data_Ihat_ig <- panel_individual |>
  filter(!is.na(HighCCEI), !is.na(Ihat_ig)) |>
  mutate(
    member = factor(
      HighCCEI,
      levels = c(0, 1),
...
ggsave(
  file.path(ihat_result_dir, "hist_Ihat_ig.png"),
  hist_Ihat_ig,
  width = 7.2,
  height = 4.6,
  dpi = 400,
...
message(
  "99_2_Figures_Main_Ihat.R completed. Ihat-dependent outputs: ",
  ihat_result_dir,
  "; unchanged outputs: ",
  result_dir
)
