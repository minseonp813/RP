################################################################################
# 99_4_Figures_Appendix_Ihat.R
# Standalone appendix-figure replication using Ihat_ig where applicable.
#
...
ihat_output_path <- function(stem, extension = ".png") {
  file.path(ihat_result_dir, paste0(stem, "_Ihat", extension))
}
...
hist_bargaining_index <- panel_individual |>
  filter(!is.na(.data[[distance_var]])) |>
  ggplot(aes(.data[[distance_var]])) +
  geom_histogram(
    binwidth = 0.05,
    boundary = 0,
    closed = "left",
  ...
  labs(
    x = if (distance_var == "Ihat_ig") {
      expression("Revealed bargaining index (" * hat(I)[ig] * ")")
    } else {
      expression("Revealed bargaining index (" * I[ig] * ")")
    },
    y = "Count"
  ) +
...
ggsave(
  ihat_output_path("hist_bargaining_index"),
...
ggsave(
  regular_output_path("risk_preference_distance_cdf_baseline"),
  absolute_ra_cdf_baseline, width = 7, height = 5, dpi = 300
)
ggsave(
  regular_output_path("risk_preference_distance_cdf_endline"),
...
ggsave(
  regular_output_path("ra_distance_by_ccei_bar"),
  ra_bar, width = 6, height = 5, dpi = 300
)
ggsave(
  regular_output_path("ra_distance_by_ccei_cdf"),
...
make_shapley_plot(
  ihat_output_path("shapley_bargaining_index", ".csv"),
  ihat_output_path("shapley_bargaining_index"),
  bargaining_order,
  c(
    "CCEI" = "white",
...
make_shapley_plot(
  regular_output_path("shapley_collective_ccei", ".csv"),
  regular_output_path("shapley_collective_ccei"),
  collective_order,
  c(
    "CCEI" = "white",
...
message(
  "99_4_Figures_Appendix_Ihat.R completed. Ihat-dependent outputs: ",
  ihat_result_dir,
  "; unchanged outputs: ",
  result_dir
)
