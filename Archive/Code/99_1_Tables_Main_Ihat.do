********************************************************************************
* 99_1_Tables_Main_Ihat.do
* Standalone main-paper table replication using Ihat_ig.
*
...
file open summary_file using `"`ihat_dir'/table_ccei_summary_Ihat.tex"', write replace
local bs = char(92)
local distance_tex "`bs'ensuremath{I_{ig}}"
if `"`distance_var'"' == "Ihat_ig" {
    local distance_tex "`bs'ensuremath{`bs'widehat{I}_{ig}}"
}
...
eststo clear
eststo m1: reghdfe `distance_var' HighCCEI if balanced_t3, absorb(class) vce(cluster class)
eststo m2: reghdfe `distance_var' HighCCEI $t34_group $t34_friend $t34_missing if balanced_t3, absorb(class) vce(cluster class)
eststo m3: reghdfe `distance_var' HighCCEI $t34_group $t34_friend $t34_missing $t34_ra $t34_share if balanced_t3, absorb(class) vce(cluster class)
eststo m4: reghdfe `distance_var' HighCCEI $t34_group_nogender $t34_friend $t34_missing $t34_ra $t34_share if balanced_t3, absorb(id_fe) vce(cluster class)
...
esttab using `"`ihat_dir'/table_bargainingCCEI_Ihat.tex"', replace ///
...
di as result "99_1_Tables_Main_Ihat.do completed."
di as result "Ihat-dependent outputs: `ihat_dir'"
di as result "Unchanged outputs: `tex_dir'"
