...
global friend_char = "inclass_n_friends_max inclass_n_friends_dist inclass_popularity_max inclass_popularity_dist" /// (TODO: add variables for friendship within the pair)
...
outreg2 using "results/table2.xls", append se bdec(3) tdec(3) ///
...
outreg2 using "results/table2.xls", append se bdec(3) tdec(3) ///
    ctitle(Pooled OLS) addnote("Robust SE clustered by class") 
* (TODO: Corner solution)

* TODO: New shapley decomposition table
macro drop class
global class ""
...
global group_char = "math_diff mathscore_dist_missing height_diff outgoing_diff opened_diff agreeable_diff conscientious_diff stable_diff outgoing_diff_missing opened_diff_missing agreeable_diff_missing conscientious_diff_missing stable_diff_missing i.male_i i.malepair_co"
global friend_char = "inclass_n_diff inclass_pop_diff i.friendship" /// (TODO: add variables for friendship within the pair)

eststo clear
eststo: reghdfe RA_ig HighCCEI, absorb(class) vce(cluster class)
eststo: reghdfe RA_ig HighCCEI post HighCCEI_post, absorb(class) vce(cluster class)
eststo: reghdfe RA_ig $group_char $friend_char mover, absorb(class) vce(cluster class)
eststo: reghdfe RA_ig HighCCEI post HighCCEI_post $group_char $friend_char mover, absorb(class) vce(cluster class)
eststo: reghdfe RA_ig HighCCEI $group_char $friend_char, absorb(id) vce(cluster class)
esttab, stats(N r2)
...
* Table 4
global group_char = "math_diff mathscore_dist_missing height_diff outgoing_diff opened_diff agreeable_diff conscientious_diff stable_diff outgoing_diff_missing opened_diff_missing agreeable_diff_missing conscientious_diff_missing stable_diff_missing i.male_i i.malepair_co"
global friend_char = "inclass_n_diff inclass_pop_diff" 

use "data/finalized_panel_individual_250825.dta", clear

eststo clear
eststo: reghdfe new2_I_ig HighCCEI, absorb(class) vce(cluster class)
eststo: reghdfe new2_I_ig HighCCEI post HighCCEI_post, absorb(class) vce(cluster class)
eststo: reghdfe new2_I_ig $group_char $friend_char mover, absorb(class) vce(cluster class)
eststo: reghdfe new2_I_ig HighCCEI post HighCCEI_post $group_char $friend_char mover, absorb(class) vce(cluster class)
eststo: reghdfe new2_I_ig HighCCEI $group_char $friend_char, absorb(id) vce(cluster class)
esttab, se stats(N r2) star(+ 0.1 * 0.05 ** 0.01)
