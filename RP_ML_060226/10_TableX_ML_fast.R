...
sim     <- 1       # change to 50 later (250 in the original paper)
K       <- 2       # number of folds
...
args         <- list(svmLinear2=list(type='eps-regression'),
                     ...
                     pcaNNet=list(linout = TRUE, trace = FALSE, MaxNWts=100000, maxit=300),
...
r <- foreach(t = 1:sim, .combine='cbind', .inorder=FALSE, .packages=vec.pac) %do% {
  ...
  for(i in 1:length(Y)){
    ...
    for(l in 1:length(methods)){
      ...
      fitControl   <- trainControl(method = methodML[l], number = cv[l], repeats = rep[l], allowParallel = TRUE, verboseIter=FALSE, search="random", selectionFunction=select[l])
      ...
      for (ii in 1:1){
        ...
        fitControl <- trainControl(method = "repeatedcv", number = 2, repeats = 2, allowParallel = TRUE, verboseIter=FALSE, search="random", selectionFunction="best")
...
fitControl <- trainControl(method = methodML[l_best], number = cv[l_best], repeats = rep[l_best],
                           allowParallel = TRUE, verboseIter=FALSE, search="random",
