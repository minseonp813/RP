################################################################################
# Figure: Shapley decomposition of Collective CCEI (Table 5, Column 4)
#
# Run the Table 5/Shapley block in 13_main_results.do first. That block creates
# results/shapley_collective_ccei.csv, which this script turns into the striped
# stacked-bar figure used in the appendix.
################################################################################

library(ggplot2)
library(ggpattern)

input_file <- "results/shapley_collective_ccei.csv"
output_file <- "results/shapley_collective_ccei.png"

if (!file.exists(input_file)) {
  stop(
    paste0(
      input_file,
      " does not exist. Run the Table 5/Shapley block in ",
