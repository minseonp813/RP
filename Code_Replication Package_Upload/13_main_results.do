********************************************************************************
* Generate All Main Results
* Written by Minseon Park 09012025
* Latest updated by Byunghun Hahn 07212026
********************************************************************************
...
preserve
    * Construct exact corner/midpoint shares directly from the new raw files.
...
global group_char = "mathscore_i mathscore_diff height_i height_diff outgoing_i outgoing_diff opened_i opened_diff agreeable_i agreeable_diff conscientious_i conscientious_diff stable_i stable_diff female_i_male_j male_i_female_j"
global group_char_no_gender = "mathscore_i mathscore_diff height_i height_diff outgoing_i outgoing_diff opened_i opened_diff agreeable_i agreeable_diff conscientious_i conscientious_diff stable_i stable_diff"
global friend_char = "inclass_n_friends_i inclass_n_diff inclass_popularity_i inclass_pop_diff" 
global missing_char = "mathscore_diff_missing outgoing_diff_missing opened_diff_missing agreeable_diff_missing conscientious_diff_missing stable_diff_missing"
global RA_char = "RA_i RA_diff"
global share_char = "corner_share_i corner_share_diff mid_share_i mid_share_diff"

* I_ig must be defined in both waves. This gives the common balanced sample
* described in the table note (1,114 students and 2,228 student-wave rows).
bysort id: egen n_I_ig = total(!missing(I_ig))
gen byte balanced = (n_I_ig == 2)
drop n_I_ig

* id is a string in the new data; create a numeric identifier for reghdfe.
egen long id_fe = group(id)
sort id group_id post

* LaTeX display labels only; the underlying Stata variable names are unchanged.
...
eststo clear
eststo: reghdfe I_ig HighCCEI if balanced==1, absorb(class) vce(cluster class)
...
* Retain complete pairs for which the FGARP influence index is defined in
* both waves for both members.
...
eststo clear
eststo: reghdfe f_I_ig HighF_CCEI if balanced_fgarp==1, absorb(class) vce(cluster class)
eststo: reghdfe f_I_ig HighF_CCEI $group_char $friend_char $missing_char if balanced_fgarp==1, absorb(class) vce(cluster class)
eststo: reghdfe f_I_ig HighF_CCEI $group_char $friend_char $missing_char $RA_char $share_char if balanced_fgarp==1, absorb(class) vce(cluster class)
eststo: reghdfe f_I_ig HighF_CCEI $group_char_no_gender $friend_char $missing_char $RA_char $share_char if balanced_fgarp==1, absorb(id_fe) vce(cluster class)

esttab using "Tables/table_bargainingFGARP.tex", replace ///
	b(3) se(3) stats(N r2, labels("N" "R-squared") fmt(0 3)) ///
	nogap compress star(+ 0.1 * 0.05 ** 0.01) label substitute(\_ _) ///
...
egen byte tag_fgarp_id = tag(id) if balanced_fgarp==1
egen byte tag_fgarp_pair = tag(group_id) if balanced_fgarp==1
count if balanced_fgarp==1
display "FGARP student-wave observations = " r(N)
count if tag_fgarp_id==1
display "FGARP students = " r(N)
count if tag_fgarp_pair==1
display "FGARP pairs = " r(N)


********************************************************************************
* Appendix: Table 3 robustness using reversed MaxMPI
********************************************************************************

* Retain complete pairs for which the reversed-MaxMPI influence index is
* defined in both waves for both members.
bysort id: egen n_revmax_I_ig = total(!missing(I_rev_max_mpi_ig))
gen byte complete_revmax_id = (n_revmax_I_ig == 2)
bysort group_id: egen byte balanced_revmax = min(complete_revmax_id)

label var HighRevMaxMPI "\$Higher RevMaxMPI_i\$"

eststo clear
eststo: reghdfe I_rev_max_mpi_ig HighRevMaxMPI if balanced_revmax==1, absorb(class) vce(cluster class)
eststo: reghdfe I_rev_max_mpi_ig HighRevMaxMPI $group_char $friend_char $missing_char if balanced_revmax==1, absorb(class) vce(cluster class)
eststo: reghdfe I_rev_max_mpi_ig HighRevMaxMPI $group_char $friend_char $missing_char $RA_char $share_char if balanced_revmax==1, absorb(class) vce(cluster class)
eststo: reghdfe I_rev_max_mpi_ig HighRevMaxMPI $group_char_no_gender $friend_char $missing_char $RA_char $share_char if balanced_revmax==1, absorb(id_fe) vce(cluster class)

esttab using "Tables/table_bargainingRevMaxMPI.tex", replace ///
	b(3) se(3) stats(N r2, labels("N" "R-squared") fmt(0 3)) ///
	nogap compress star(+ 0.1 * 0.05 ** 0.01) label substitute(\_ _) ///
...
egen byte tag_revmax_id = tag(id) if balanced_revmax==1
egen byte tag_revmax_pair = tag(group_id) if balanced_revmax==1
count if balanced_revmax==1
display "RevMaxMPI student-wave observations = " r(N)
count if tag_revmax_id==1
display "RevMaxMPI students = " r(N)
count if tag_revmax_pair==1
display "RevMaxMPI pairs = " r(N)
