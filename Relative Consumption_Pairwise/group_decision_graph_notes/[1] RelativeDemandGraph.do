...
use Risk_Merged_Short.dta, clear 


* Before drawing the graph, we impose a new groupid based on the natural order
* to make the looping easier; looping from 1 to 100 means we draw graphs of a group
* with smallest groupid to the group with 100th smallest groupid

egen temp=tag(groupid) // representative individual from each pair. otherwise, -rank- gets messy
sort temp groupid
egen temp1=rank(groupid) if temp==1 
gsort groupid -temp // give new groupid
bysort groupid : replace temp1=temp1[1] //copy the groupid to the unselected member 
