...
baseline_only_noncog_cols <- paste0(baseline_only_noncog_stems, "_base")
if (!all(baseline_only_noncog_cols %in% names(noncog_pre_clean))) {
  stop("Missing baseline-only noncognitive variables.")
}
...
score_rat_clean <- function(df, answers, suffix, module_name) {
  ...
  for (i in 1:10) {
    ...
    strict_scores[[i]] <- ifelse(
      ...
      0L,
      as.integer(
        str_detect(response, fixed(answer_i)) &
          str_length(response) == str_length(answer_i)
      )
    )
  }
