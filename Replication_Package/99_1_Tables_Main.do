...
eststo bh3: reghdfe `distance_var' HighCCEI_both_high $t34_group $t34_friend $t34_missing $t34_ra $t34_share if balanced_t3, absorb(class) vce(cluster class)
eststo bh4: reghdfe `distance_var' HighCCEI_both_high $t34_group_nogender $t34_friend $t34_missing $t34_ra $t34_share if balanced_t3, absorb(id_fe) vce(cluster class)
...
eststo cd3: reghdfe `distance_var' ccei_gap_ij $t34_group $t34_friend $t34_missing $t34_ra $t34_share if balanced_t3, absorb(class) vce(cluster class)
eststo cd4: reghdfe `distance_var' ccei_gap_ij $t34_group_nogender $t34_friend $t34_missing $t34_ra $t34_share if balanced_t3, absorb(id_fe) vce(cluster class)
...
foreach definition in bothhigh bothlow cceidiff {
    if "`definition'" == "bothhigh" {
        local ccei_var "HighCCEI_both_high"
        local file_suffix ""
    }
    else if "`definition'" == "bothlow" {
        local ccei_var "HighCCEI_both_low"
        local file_suffix "_bothlow"
    }
    else {
        local ccei_var "ccei_gap_ij"
        local file_suffix "_cceidiff"
    }

    eststo clear
    eststo: reghdfe RA_I_ig `ccei_var', absorb(class) vce(cluster class)
    eststo: reghdfe RA_I_ig `ccei_var' $t34_group $t34_friend $t34_missing, absorb(class) vce(cluster class)
    eststo: reghdfe RA_I_ig `ccei_var' $t34_group $t34_friend $t34_missing $t34_ra $t34_share, absorb(class) vce(cluster class)
    eststo: reghdfe RA_I_ig `ccei_var' $t34_group_nogender $t34_friend $t34_missing $t34_ra $t34_share, absorb(id_fe) vce(cluster class) keepsingletons
    esttab using `"`tex_dir'/table_bargainingRA_distance`file_suffix'.tex"', replace ///
        b(3) se(3) stats(N r2, labels("N" "R-squared") fmt(0 3)) ///
        nogap compress star(+ 0.1 * 0.05 ** 0.01) label substitute(\_ _) ///
        keep(`ccei_var' mathscore_i mathscore_diff inclass_popularity_i inclass_pop_diff female_i_male_j male_i_female_j) ///
